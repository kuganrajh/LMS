...
using WLV.LMS.WEBAPI.Providers;
using WLV.LMS.WEBAPI.Results;
...
namespace WLV.LMS.WEBAPI.Controllers
{
    ...
    public class AccountController : ApiController
    {
        ...
        public async Task<HttpResponseMessage> ChangePassword(ChangePasswordBindingModel model)
        {
            ...
            return Request.CreateResponse<ApplicationResult>(HttpStatusCode.OK, new ApplicationResult() { Result = 0 });
        }
        ...
        public async Task<IHttpActionResult> GetExternalLogin(string provider, string error = null)
        {
            ...
            string UserRoles = String.Join(", ", UserManager.GetRoles(user.Id).ToArray());
        ...
        public async Task<HttpResponseMessage> Register(RegisterBindingModel model)
        {
            ...
            if (!result.Succeeded)
            {
                ApplicationResult applicationResult = new ApplicationResult();
                applicationResult.Status.IsError = true;
                applicationResult.Status.ErrorMessage = string.Format("User Creation Failed");
                return Request.CreateResponse<ApplicationResult>(HttpStatusCode.ExpectationFailed, applicationResult);
            }            
            return Request.CreateResponse<ApplicationResult>(HttpStatusCode.OK, new ApplicationResult() { Result = user.Id });
        }
